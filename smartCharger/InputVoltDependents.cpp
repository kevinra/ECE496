...
#define NUMOFPOLLSTRUCT (NUMOFINTERRUPT + 1)
...
int InputVoltDependents::init()
{
  ...
  {
    DBG_ERR_MSG("Beaglebone input from FPGA output pin #2 initializing failed!");
     // g_errQueue.addErrStr("");
  }
  if ( m_pGPIO_bIn_fOut_1->init(INPUT_PIN, STR_RISING_EDGE, true) )
  {
    DBG_ERR_MSG("Beaglebone input from FPGA output pin #1 initializing failed!");
     // g_errQueue.addErrStr("");
  }
  if ( m_pGPIO_bIn_fOut_0->init(INPUT_PIN, STR_RISING_EDGE, true) )
  {
    DBG_ERR_MSG("Beaglebone input from FPGA output pin #0 initializing failed!");
     // g_errQueue.addErrStr("");
  }

  // For some reason, when interrupt is setup initially,
  // it triggers the interrupt so run it on initialization.
  waitForInterrupt();
  return 0;
}
...
void* InputVoltDependents::run()
{
  ...
  while ( !g_EVStateNInputVInterface.getIsChargingHWOoS() )
  {
    if ( isBothLdEnReady() )
    {
      g_EVStateNInputVInterface.setShouldLLCOn(true);
      waitForInterrupt();
    }
  }
...
void InputVoltDependents::waitForInterrupt()
{
  ...
  if (fdset[INTERRUPTNUM_REC1PFM].revents & POLLPRI)
  {
    ...
    g_EVStateNInputVInterface.setShouldLLCOn(false);
  }
  ...
  if (failedLLCNum)
  {
    DBG_ERR_MSG("LLC #" << failedLLCNum << "failed!");
